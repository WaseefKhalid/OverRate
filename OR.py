...
# App title
st.title("Cricket Over Completion Tracker")
...
start_time_str = st.text_input("Enter the start time of the match (HH:MM, 24-hour format):", value="14:00")
...
if st.button("Generate Over Completion Schedule") and start_time:
    ...
    for over in range(1, total_overs + 1):
        ...
        over_schedule.append((over, over_end_time.strftime("%I:%M %p")))

    # Display the schedule
    st.subheader("Over Completion Schedule")
    for over, time in over_schedule:
        st.write(f"Over {over} should end by: **{time}**")
